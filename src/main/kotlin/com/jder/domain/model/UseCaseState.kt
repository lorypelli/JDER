...
class UseCaseState {
  ...
  fun undo() {
    if (undoStack.isNotEmpty()) {
      ...
      isModified = true
      if (selectedActorId != null && diagram.actors.none { it.id == selectedActorId })
          selectedActorId = null
      if (selectedUseCaseId != null && diagram.useCases.none { it.id == selectedUseCaseId })
          selectedUseCaseId = null
      if (selectedRelationId != null && diagram.relations.none { it.id == selectedRelationId })
          selectedRelationId = null
      if (selectedNoteId != null && diagram.notes.none { it.id == selectedNoteId })
          selectedNoteId = null
      if (
          selectedSystemBoundaryId != null &&
              diagram.systemBoundaries.none { it.id == selectedSystemBoundaryId }
      )
          selectedSystemBoundaryId = null
    }
  }
  ...
  fun addActor(x: Float, y: Float, name: String) {
    ...
    diagram =
        diagram.copy(
            actors =
                diagram.actors + Actor(id = UUID.randomUUID().toString(), name = name, x = x, y = y)
        )
    isModified = true
  }
  ...
  fun addUseCase(x: Float, y: Float, name: String) {
    ...
    diagram =
        diagram.copy(
            useCases =
                diagram.useCases +
                    UseCase(id = UUID.randomUUID().toString(), name = name, x = x, y = y)
        )
    isModified = true
  }
  ...
  fun addNote(x: Float, y: Float, text: String) {
    ...
    diagram =
        diagram.copy(
            notes =
                diagram.notes + Note(id = UUID.randomUUID().toString(), text = text, x = x, y = y)
        )
    isModified = true
  }
  ...
  fun addSystemBoundary(x: Float, y: Float, name: String) {
    ...
    diagram =
        diagram.copy(
            systemBoundaries =
                diagram.systemBoundaries +
                    SystemBoundary(id = UUID.randomUUID().toString(), name = name, x = x, y = y)
        )
    isModified = true
  }
  ...
  fun updateUseCase(useCaseId: String, update: (UseCase) -> UseCase) {
    saveState()
    diagram =
        diagram.copy(useCases = diagram.useCases.map { if (it.id == useCaseId) update(it) else it })
    isModified = true
  }
  ...
  fun updateRelation(relationId: String, update: (UseCaseRelation) -> UseCaseRelation) {
    ...
    diagram =
        diagram.copy(
            relations = diagram.relations.map { if (it.id == relationId) update(it) else it }
        )
    isModified = true
  }
  ...
  fun updateSystemBoundary(boundaryId: String, update: (SystemBoundary) -> SystemBoundary) {
    ...
    diagram =
        diagram.copy(
            systemBoundaries =
                diagram.systemBoundaries.map { if (it.id == boundaryId) update(it) else it }
        )
    isModified = true
  }

  fun updateSystemBoundaryWithoutSave(
      boundaryId: String,
      update: (SystemBoundary) -> SystemBoundary,
  ) {
    diagram =
        diagram.copy(
            systemBoundaries =
  ...
  fun deleteActor(actorId: String) {
    ...
    diagram =
        diagram.copy(
            ...
            relations =
                diagram.relations.filter { it.sourceId != actorId && it.targetId != actorId },
        )
  ...
  fun deleteUseCase(useCaseId: String) {
    ...
    diagram =
        diagram.copy(
            ...
            relations =
                diagram.relations.filter { it.sourceId != useCaseId && it.targetId != useCaseId },
        )
  ...
  fun deleteSystemBoundary(boundaryId: String) {
    saveState()
    diagram =
        diagram.copy(systemBoundaries = diagram.systemBoundaries.filter { it.id != boundaryId })
