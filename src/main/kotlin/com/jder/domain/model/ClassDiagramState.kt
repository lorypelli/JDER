...
class ClassDiagramState {
  ...
  private fun saveState() {
    undoStack.add(
        diagram.copy(
            classes =
                diagram.classes.map {
                  it.copy(attributes = it.attributes.toList(), methods = it.methods.toList())
                },
  ...
  fun undo() {
    if (undoStack.isNotEmpty()) {
      ...
      isModified = true
      if (selectedClassId != null && diagram.classes.none { it.id == selectedClassId })
  ...
  fun updateClass(classId: String, update: (ClassEntity) -> ClassEntity) {
    saveState()
    diagram =
        diagram.copy(classes = diagram.classes.map { if (it.id == classId) update(it) else it })
    isModified = true
  }
  ...
  fun deleteClass(classId: String) {
    ...
    diagram =
        diagram.copy(
            ...
            relations =
                diagram.relations.filter { it.sourceId != classId && it.targetId != classId },
        )
