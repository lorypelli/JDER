...
private fun drawRelation(
  ...
  when (rel.type) {
    UseCaseRelationType.ASSOCIATION -> {
      g.stroke = BasicStroke(1.5f)
      g.drawLine(
          (src.first + offX).toInt(),
          (src.second + offY).toInt(),
          (tgt.first + offX).toInt(),
          (tgt.second + offY).toInt(),
      )
    }
...
private fun getCenter(id: String, diagram: UseCaseDiagram): Pair<Float, Float>? {
  diagram.actors
      .find { it.id == id }
      ?.let {
        return Pair(it.x + it.width / 2, it.y + it.height / 2)
      }
  diagram.useCases
      .find { it.id == id }
      ?.let {
