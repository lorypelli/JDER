...
private fun drawClassEntityR(g: Graphics2D, entity: ClassEntity, offX: Float, offY: Float) {
  ...
  g.drawString(
      entity.name,
      x + (w - nameW) / 2,
      currentY + (if (entity.type.stereotype.isNotEmpty()) 18 else 25),
  )
  currentY = y + headerH
  g.color = Color(0x3F51B5)
...
private fun drawClassRelationR(
    g: Graphics2D,
    rel: ClassRelation,
  ...
  if (rel.sourceMultiplicity.isNotEmpty()) {
    ...
    g.drawString(
        ...
        (src.first + nx * 20 + offX).toInt(),
        (src.second + ny * 20 + offY).toInt(),
    )
  }
  if (rel.targetMultiplicity.isNotEmpty()) {
    ...
    g.drawString(
        ...
        (tgt.first - nx * 24 + offX).toInt(),
        (tgt.second - ny * 24 + offY).toInt(),
    )
  }
}
...
private fun drawOpenArrowR(g: Graphics2D, tipX: Float, tipY: Float, nx: Float, ny: Float) {
  ...
  g.drawLine(
      tipX.toInt(),
      tipY.toInt(),
      (baseX + px * size * 0.4f).toInt(),
      (baseY + (-nx) * size * 0.4f).toInt(),
  )
  g.drawLine(
      tipX.toInt(),
      tipY.toInt(),
...
private fun drawHollowTriangleR(g: Graphics2D, tipX: Float, tipY: Float, nx: Float, ny: Float) {
  ...
  val xs =
      intArrayOf(
          tipX.toInt(),
          (baseX + px * size * 0.5f).toInt(),
          (baseX - px * size * 0.5f).toInt(),
      )
  val ys =
      intArrayOf(
          tipY.toInt(),
          (baseY + (-nx) * size * 0.5f).toInt(),
          (baseY - (-nx) * size * 0.5f).toInt(),
      )
  g.color = Color.WHITE
