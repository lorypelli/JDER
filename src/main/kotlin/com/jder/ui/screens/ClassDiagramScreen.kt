...
fun ClassDiagramScreen(
              ...
              when {
                ...
                event.key == Key.Delete || event.key == Key.Backspace -> {
                  state.selectedClassId?.let {
                    ...
                    snackbarMessage = "Classe eliminata"
                  }
        ...
        if (showContextMenu) {
          ClassDiagramContextMenu(
              ...
              onDelete = {
                showContextMenu = false
                when (contextMenuType) {
                  ClassDiagramContextMenuType.CLASS ->
                      state.selectedClassId?.let {
                        state.deleteClass(it)
                        snackbarMessage = "Classe eliminata"
                      }
                  ClassDiagramContextMenuType.RELATION ->
                      state.selectedRelationId?.let {
                        state.deleteRelation(it)
                        snackbarMessage = "Relazione eliminata"
                      }
                  ClassDiagramContextMenuType.NOTE ->
                      state.selectedNoteId?.let {
                        state.deleteNote(it)
                        snackbarMessage = "Nota eliminata"
                      }
        ...
        Surface(modifier = Modifier.width(300.dp).fillMaxHeight(), tonalElevation = 2.dp) {
          ClassDiagramPropertiesPanel(
              ...
              onDeleteMember = { memberId ->
                state.selectedClassId?.let {
                  state.deleteMemberFromClass(it, memberId)
                  snackbarMessage = "Membro eliminato"
                }
              },
  ...
  if (showClassDialog) {
    state.diagram.classes
        .find { it.id == state.selectedClassId }
        ?.let { entity ->
          ClassEntityDialog(
              name = entity.name,
              type = entity.type,
        ...
        ?.let { relation ->
          ClassRelationDialog(
              ...
              onConfirm = { newType, srcMulti, tgtMulti, lbl ->
                state.updateRelation(relation.id) {
                  it.copy(
                      type = newType,
  ...
  if (showExportDialog) {
    FileManagerDialog(
        ...
        onFileSelected = { file ->
          ...
          try {
            ImageExporter()
                .exportToPNG(renderClassDiagramToBitmap(state.diagram), pngFile)
                .fold(
