  ...
  when (state.toolMode) {
    ToolMode.SELECT -> {
      state.diagram.relationships
          .firstOrNull { isPointInDiamond(offset, it.x, it.y, it.width, it.height) }
          ?.let {
      ...
      state.diagram.entities
          .firstOrNull {
            offset.x >= it.x &&
                offset.x <= it.x + it.width &&
                offset.y >= it.y &&
                offset.y <= it.y + it.height
          }
          ?.let {
      ...
      state.diagram.notes
          ...
          ?.let { state.selectNote(it.id) } ?: run { state.clearSelection() }
    }
    ToolMode.ENTITY -> {
...
internal fun handleDragStart(
  ...
  state.diagram.entities.forEach { entity ->
    entity.attributes.forEachIndexed { index, attribute ->
      ...
      val distance =
          sqrt((adjusted.x - attrPos.x).let { it * it } + (adjusted.y - attrPos.y).let { it * it })
      if (distance <= 30f) {
  ...
  state.diagram.entities
      .find {
        adjusted.x >= it.x &&
            adjusted.x <= it.x + it.width &&
            adjusted.y >= it.y &&
            adjusted.y <= it.y + it.height
      }
      ?.let {
        state.selectEntity(it.id)
        state.saveDragStartState()
  ...
  state.diagram.relationships
      .find { isPointInDiamond(adjusted, it.x, it.y, it.width, it.height) }
      ?.let {
        state.selectRelationship(it.id)
        state.saveDragStartState()
        return Triple(null, null, null)
  ...
  draggedAttributeId?.let { attrId ->
        ...
        entity.copy(
            attributes =
                entity.attributes.mapIndexed { index, attr ->
                  ...
                  val fixedDistance =
                      if (currentDistFromCenter > 0) currentDistFromCenter
                      else (entity.width / 2 + 60f)
    ...
    draggedAttributeForRelationship?.let { relId ->
      state.updateRelationshipWithoutSave(relId) { rel ->
        rel.copy(
            attributes =
                rel.attributes.mapIndexed { index, attr ->
                  if (attr.id != attrId) return@mapIndexed attr
                  val centerX = rel.x + rel.width / 2
                  val centerY = rel.y + rel.height / 2
                  val currentAttrX =
                      if (attr.x == 0f && attr.y == 0f) rel.x + rel.width + 60f
                      else centerX + attr.x
                  val currentAttrY =
                      if (attr.x == 0f && attr.y == 0f) {
