...
internal fun DrawScope.drawActor(
  ...
  drawText(
      ...
      topLeft = Offset(cx - nameResult.size.width / 2, actor.y + actor.height + 4f),
  )
}
...
internal fun DrawScope.drawUseCaseRelation(
  ...
  val normY = dy / dist
  val sourcePoint =
      getElementBoundaryPoint(relation.sourceId, diagram, Offset(normX, normY)) ?: sourceCenter
  val targetPoint =
      getElementBoundaryPoint(relation.targetId, diagram, Offset(-normX, -normY)) ?: targetCenter
  val arrowSize = 13f
  when (relation.type) {
    UseCaseRelationType.ASSOCIATION -> {
      drawLine(color = lineColor, start = sourcePoint, end = targetPoint, strokeWidth = strokeWidth)
    }
    UseCaseRelationType.INCLUDE,
    UseCaseRelationType.EXTEND -> {
      ...
      val labelResult =
          textMeasurer.measure(
              ...
              style =
                  TextStyle(color = lineColor, fontSize = 11.sp, fontWeight = FontWeight.Normal),
          )
...
internal fun getElementCenter(id: String, diagram: UseCaseDiagram): Offset? {
  diagram.actors
      .find { it.id == id }
      ?.let {
        return Offset(it.x + it.width / 2, it.y + it.height / 2)
      }
  diagram.useCases
      .find { it.id == id }
      ?.let {
...
internal fun getElementBoundaryPoint(
    id: String,
    diagram: UseCaseDiagram,
    dirFromCenter: Offset,
): Offset? {
  diagram.actors
      .find { it.id == id }
      ?.let { actor ->
        ...
        return Offset(
            cx + dirFromCenter.x * minOf(scaleX, scaleY),
            cy + dirFromCenter.y * minOf(scaleX, scaleY),
        )
      }
  diagram.useCases
      .find { it.id == id }
