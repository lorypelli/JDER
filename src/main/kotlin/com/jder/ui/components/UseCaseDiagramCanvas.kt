  ...
  Canvas(
      modifier =
          modifier
              ...
              .pointerInput(
                  state.diagram.actors.size,
                  state.diagram.useCases.size,
                  state.diagram.notes.size,
                  state.diagram.systemBoundaries.size,
              ) {
                detectTapGestures(
                    onTap = { offset ->
                    ...
                    onLongPress = { offset ->
                      ...
                      val clickedUseCase =
                          if (clickedActor == null)
                              state.diagram.useCases.find { isPointInUseCase(adjusted, it) }
                          else null
                      val clickedRelation =
                          if (clickedActor == null && clickedUseCase == null) {
                            state.diagram.relations.find {
                              isPointNearRelation(adjusted, it, state.diagram)
                            }
                          } else null
                      val clickedNote =
                          if (
              ...
              .pointerInput(
                  ...
                  state.selectedSystemBoundaryId,
              ) {
                detectDragGestures(
                    onDragStart = { offset ->
                      isDragging = false
                    ...
                    onDragEnd = { if (!isDragging) totalDragDistance = 0f },
                )
              }
  ) {
